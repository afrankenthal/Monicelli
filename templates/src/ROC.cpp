...
bool ROC::calibratePixel(int row, int col, int adc, int& charge, bool isDut, bool verbose)
{
    ...
    //ANDRE Revert COL HERE TO GET THE RIGHT CALIBRATION
//    if (col % 6 == 5) col = (col + 1) / 6 - 1;
//    else if (col % 6 == 0) col = col / 6;
//    else std::cout << "ERROR-ERROR!!" << std::endl;
    ...
    if (par != 0)
    {
        ...
        if (verbose) {
            std::cout<<"\nWSI newAdc: "<<newAdc[0]<<" par[0]"<<par[0]<<" par[1]: "<<par[1]<<" => charge: "<<charge<<std::endl;
        }
